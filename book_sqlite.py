...
def reg_book_db():
    ...
    print('件数', isbn_num[0]['isbn_cnt'])
    # ISBNが空文字ではない and 重複した場合にエラーを発生
    if isbn != '' and int(isbn_num[0]['isbn_cnt']) >= 1:
...
def list_book():
    ...
    # DBの情報を表示
    cur.execute('''
        SELECT id, isbn, title, author, publisher, pubdate, person, substr(created_at, 0, 11) AS created_at, updated_at
        FROM books
    ...
    record_num = int(record_data[0]['record_cnt'])
    cur.close()
    con.close()
    # ページネーションの設定
