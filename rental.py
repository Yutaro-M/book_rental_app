...
def list_rental():
    ...
    cur = con.cursor()
    cur.execute('''
        SELECT rental.user_id, user_name, book_id, title, to_char(rental.created_at, 'yyyy-mm-dd') as created_at, to_char(returned_at, 'yyyy-mm-dd') as returned_at
        FROM rental
        INNER JOIN users
    ...
    rental_lst = [dict(zip(keys, row)) for row in rental_lst]
    cur.close()
    con.close()
...
def return_rental():
    ...
    cur.execute('''
        UPDATE rental
        SET returned_at = now(), person_return = ?
        WHERE user_id = ? AND book_id = ? AND returned_at IS NULL
    ...
    # 情報を表示
    cur.execute('''
        SELECT rental.user_id, user_name, book_id, title, to_char(rental.created_at, 'yyyy-mm-dd') as created_at, to_char(rental.returned_at, 'yyyy-mm-dd') as returned_at
        FROM rental
        INNER JOIN users
        ON rental.user_id = users.user_id
        INNER JOIN books
