...
def reg_user_db():
    ...
    # pg8000を使用して、カラム名にアクセスするために必要な2行
    keys = [k[0] for k in cur.description]
    user_id_num  = [dict(zip(keys, row)) for row in user_id_num ]
    if int(user_id_num[0]['user_id_cnt']) >= 1:
    # if int(user_id_num[0][0]) >= 1:
...
def list_user():
    ...
    cur = con.cursor()
    cur.execute('''
        SELECT user_id, user_name, user_email, person, to_char(created_at, 'yyyy-mm-dd') as created_at, updated_at
        FROM users
    ...
    keys = [k[0] for k in cur.description]
    user_lst = [dict(zip(keys, row)) for row in user_lst]
    cur.close()
    con.close()
